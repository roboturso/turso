...
use crate::sync::Arc;
use crate::{Completion, CompletionError, OpenFlags};
...
impl FaultTruncateFile {
    fn new(inner: Arc<dyn crate::File>) -> Self {
        Self {
            inner,
...
impl crate::File for FaultTruncateFile {
...
fn row_version(commit_ts: u64) -> crate::mvcc::database::RowVersion {
    let table_id: crate::mvcc::database::MVTableId = (-100).into();
    crate::mvcc::database::RowVersion {
        ...
        row: crate::mvcc::database::tests::generate_simple_string_row(table_id, 1, "issue-7993"),
...
fn log_with_one_durable_frame(
    ...
    let file: Arc<dyn crate::File> = fault.clone();
    let mut log = LogicalLog::new(file, io.clone(), None);

    let tx = crate::mvcc::database::LogRecord::for_test(10, &[row_version(10)], None);
